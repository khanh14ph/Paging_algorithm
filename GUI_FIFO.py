...
def create_label():
    ...
    for index1,j in enumerate(i):
        ttk.Label(root, text=str(j),font=("Helvetica", word_size), borderwidth=10, relief="solid").grid(row=index1+1, column=temp+1)
    ttk.Label(root,text=pf,font=("Helvetica", word_size), borderwidth=10, relief="solid").grid(row=frames+1, column=temp+1)
    temp+=1
temp=0
frames=3
lst=[3,2,1,2,4,1,5]
...
for i in range(frames):
    ttk.Label(root, text=f"f{i}",font=("Helvetica", word_size),borderwidth=1, relief="solid").grid(row=i+1, column=0)
